# Discord Bot -- Cryptocurrency Notifications
...
- Author: Aiden Zepp
- 'README.md' file updated as of: 19 March, 2021.
...
In order to retrieve data from **CoinMarketCap**, the [Requests](https://pypi.org/project/requests/) Python library must be installed.
...
The bot's code uses a `.gitignore` file to hide files containing unnecessary or sensitive information. Filepaths mentioned within the author's `.gitignore` file won't show up in this GitHub repository. This is due to privacy reasons, such as how the bot's Discord and CoinMarketCap API keys are stored in a file hidden from the GitHub repository. However, any files needed that are hidden from GitHub will be described along with the process to format and create the files correctly.

If the `.gitignore` file is needed, below are the steps to create and format the `.gitignore` file. If it's not needed, files can still be accessed by the bot without a problem, however, the files won't be shown in this GitHub repository.
...
As seen in the example above, `src/hidden` is a folder named `hidden` that is being stored in the `src` directory. For this source code, many files that are specific to the user accessing the bot were stored in `src/hidden` -- as long as they were able to be stored in `src/hidden`. The `secrets.json` file stored within the `hidden` folder is holding information for the API keys, a single server's information, and preferences for startup and bot usage. Below shows the format used for the source `secrets.json` file.
```sh
1   {
2      "keys": {
...
- The server's name -- seen as `"EXAMPLE_SERVER_NAME"` in the example above -- would be replaced with the name of the server that the bot is being used in (the bot was only needed by the author to be used in one server).
- The server's ID -- seen as `"EXAMPLE_SERVER_ID"` in the example above -- would be replaced with the ID of the server that the bot is being used in (the bot was only needed by the author to be used in one server).
...
- **Type:** *file*
- **PATH:**
    - **Parent Path**: *src/hidden*
    - **File Path**: *src/hidden/secrets.json*
- **Format Reference:**
    ```sh
    1   {
    2      "keys": {
...
- **PATH:**
    - **Parent Path**: *src/hidden*
    - **File Path**: *src/hidden/ALL_USERS_INFO*
- **Child Folders & Files:**
    - Contains all user files for the members in the server.
    - All user files are named based on the corresponding member's Discord ID (e.g. `0123456789.json`).

#### III. 'CMC_DATA':
- **Type:** *folder*
- **PATH:**
    - **Parent Path**: *src/hidden*
    - **File Path**: *src/hidden/CMC_DATA*
- **Child Folders & Files:** 
...
- **Type:** *file*
- **PATH:**
    - **Parent Path**: *src/hidden/CMC_DATA*
    - **File Path**: *src/hidden/CMC_DATA/CMC_DATA.json*

##### III.II 'CMC_DATA_REQUEST.json':
- **Type:** *file*
- **PATH:**
    - **Parent Path**: *src/hidden/CMC_DATA*
    - **File Path**: *src/hidden/CMC_DATA/CMC_DATA_REQUEST.json*
- **Format Reference:**
```sh
{
    "requestinfo": {
...
```
- `LIMIT_VALUE` would be replaced with the number of cryptocurrencies that should be stored (leave in string form; look at the **FAQ** section about setting `limit`)
- `REAL_WORLD_CURRENCY` would be replcaed with the real world currency code desired (e.g. USD, EUR, CNY)
- `API_KEY` would be replaced with the CoinMarketCap API key.

#

## List of Commands:

The commands listed below are **only callable commands**, meaning that the commands listed are only ones which can be called in a Discord server. Commands that are not able to be directly called in a Discord server are not listed. Each command is either within the main `bot.py` file or within a cog file. The type of file for each section header is depicted by **[MAIN]** (representing commands within the main `bot.py` file) or **[COG]** (representing commands within its respective cog file).
...
When starting up the bot, text will be printed to the terminal asking primarily 'Yes' or 'No' questions about the startup process. The bot references the `secrets.json` file for stored preferences, and it displays the current settings before asking any questions.

- `1. Run Confirmations?`
    - **Description:** asks whether the bot should run through the confirmation questions before fully starting up and activating the bot.
    - **Response:** *Y*/*Yes* or *N*/*No*
        > **Note:** if the response entered is in some form of 'No', then the bot will load everything using the settings stored.
    ...
    - **Example:**
        ...
        Run confirmations? (y/n) [If not, the bot will startup immediately.]  |  Answer: 
        ```
...
- `2. Start the bot?`
    ...
    - **Response:** *Y*/*Yes* or *N*/*No*
        > **Note:** if the response entered is in some form of 'No', then the bot will immediate exit the startup process.

    - **Example:**
        ```sh
        [Confirmation] Would you like to start the bot? (y/n)   |  Answer: 
        ```

- `3. Prefix?`
    - **Description:** confirms that the bot's command prefix stored should remain as the prefix.
    - **Response:** *Y*/*Yes* or *N*/*No*
        > **Note:** if the response entered is in some form of 'No', then the bot will ask what the new prefix should be.

    - **Example:**
        ```
        [Confirmation] Would you like to keep the command prefix set to '{prefix}'? (y/n)   |  Answer: 
        ```

- `4. Change Prefix`
    - **Description:** only appears if the question above has a response entered in some form of 'No'; asks what character(s) the bot's command prefix should be set to.
    - **Response:** the character(s) the bot's command prefix should be set to. 
    - **Example:**
        ```
        What would you like the command prefix to be set to?   |  Prefix: 
        ```

- `5. Prefix Permanence?`
    - **Description:** asks whether the newly set prefix should be a permanent change.
    - **Response:** *Y*/*Yes* or *N*/*No*
        > **Note:** if the response entered is in some form of 'No', then the bot will keep the prefix stored in the `secrets.json` file and will continue using that stored prefix the next time the bot is started up.

    - **Example:**
        ```
        Would you like the specified prefix to be a permanent change? (y/n)   |  Answer: 
        ```
...
- `6. Load Data on Startup?`
    ...
    - **Response:** *Y*/*Yes* or *N*/*No*
        > **Note:** as it states in the terminal, the response entered will be stored permanently; if the response entered is in some form of 'No', then the bot will **not** load the data on startup.

    - **Example:**
        ```
        ** Note: The following decision will be saved for future reference. **
        [Confirmation] Would you like to load the CoinMarketCap data on startup? (y/n)   |  Answer: 
        ```
...
If the name of the JSON file containing the API keys is a different name than `secrets.json`, be sure to change the filepath within the `# -- Load Secrets --` section in the `bot.py` or `helper.py` files; an example form of the code referenced can be found below.
```sh
# -- Load Secrets --
...
Within the parentheses of `open()` in the code above, enter the filepath being used to store the keys. In the code above taken from the source code, the filepath is `src/hidden/secrets.json`, and if this remains as the filepath then none of the source code needs to be altered. If, for example, if only the file's name was changed and it was changed to `keys.json`, the code would look like the example code below.
```sh
# -- Load Secrets --
...
To change the data-startup preference, change the boolean value in the code below (found in the `secrets.json` file, stored within the `"datastartup"` key).
```sh
[...]
6      "server": {
7         "name": "EXAMPLE_SERVER_NAME",
...
12  }
```
> If the `secrets.json` file uses the above example line of code, the bot will load the CoinMarketCap data on startup.
...
*CoinMarketCap* base-level accounts that are using API keys are only able to convert to a single real-world currency at a time. An upgraded *CoinMarketCap* account would be needed in order to use `convert` with more than one real-world currency.
...
[...]
```
> If the `CMC_DATA_REQUESTS.json` file uses the above example code, each cryptocurrency would contain information relating to the conversion to USD.
